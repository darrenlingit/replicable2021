...
```{r}
# some testing
meta_prior <- prior_prp(beta = as.double(meta_beta[3, ]),
                        se = as.double(meta_se[3, ]))

list_eff = lapply(seq_len(nrow(meta_beta)),
                    function(x) {
                      meta_beta <- as.double(meta_beta[x,])
                      meta_beta <- meta_beta[is.na(meta_beta) == FALSE]
                      #beta <- beta[is.infinite(beta) == FALSE]
                    })

list_se = lapply(seq_len(nrow(meta_se)),
                   function(x) {
                     meta_se <- as.double(meta_se[x,])
                     meta_se <- meta_se[is.na(meta_se) == FALSE]
