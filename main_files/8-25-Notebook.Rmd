...
```{r Forest Plot All SNPs, echo = FALSE}
row_n <- 1

df_beta <- t(height_rep_df[row_n, 5:9])
row.names(df_beta) <- c("AA", "EA", "EAS", "HA", "SA")
df_se <- t(height_rep_df[row_n, 10:14])

chr_id <- height_rep_df[row_n, 1]
pos_id <- height_rep_df[row_n, 2]
snp_id <- paste("CHR: ", chr_id, ", ", "POS:", pos_id, sep = "")

mamba_val <- height_rep_df[row_n, 3]
prp_val <- height_rep_df[row_n, 4]
...
In contrast, below we have a SNP that the replicability packages have determined as not replicable. There is significantly less overlap.
